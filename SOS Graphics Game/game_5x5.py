...
pygame.display.set_caption("SOS 5x5")
...
def choose_letter(i, j):
    ...
    if mouse[0] < grid[i][j][0] + SQUARE_SIZE_5//2 + (WIDTH - BOARD_WIDTH)//2:
        draw_text("S", letter_font, BLACK, board_surface, grid[i][j][0] + SQUARE_SIZE_5//2 - 20, grid[i][j][1] + SQUARE_SIZE_5//2 - 20)
        return "S"
    else:
        draw_text("O", letter_font, BLACK, board_surface, grid[i][j][0] + SQUARE_SIZE_5//2 - 20, grid[i][j][1] + SQUARE_SIZE_5//2 - 20)
        return "O"
...
def check_o_points(col, row):
    ...
    if ROWS_5 -1> row > 0 and 0 <= col < COLS_5:
        if grid[col][row] == "O" and grid[col][row-1] == "S" and grid[col][row+1] == "S":
            if rounds %2 == 0:
                ...
                red_score += 1
            play_again = True
    if COLS_5-1> col > 0 and 0 <= row < ROWS_5:     
        if grid[col][row] == "O" and grid[col-1][row] == "S" and grid[col+1][row] == "S":
            if rounds %2 == 0:
                ...
                red_score += 1
            play_again = True
    if 0 < row <ROWS_5-1 and 0< col < COLS_5-1:
    ...
    if 1< row <ROWS_5:
        if grid[col][row - 1] == "O" and grid[col][row - 2] == "S":
            if rounds %2 == 0:
                ...
                red_score += 1
            play_again = True
    if ROWS_5>row >1 and 0 <= col <ROWS_5-2:
        if grid[col+1][row - 1] == "O" and grid[col+2][row - 2] == "S":
            if rounds %2 == 0:
                ...
                red_score += 1
            play_again = True
    if ROWS_5 -2>row >=0 and 0 <= col <ROWS_5-2:
        if grid[col+1][row+1] == "O" and grid[col+2][row+2] == "S":
            if rounds %2 == 0:
                blue_score += 1
            else:
                red_score += 1
            play_again = True
    if ROWS_5 -2>row >=0 and 1 < col <ROWS_5:
        if grid[col-1][row+1] == "O" and grid[col-2][row+2] == "S":
            if rounds %2 == 0:
                blue_score += 1
            else:
                red_score += 1
            play_again = True
    if ROWS_5>row >1 and 1 < col <ROWS_5:
        if grid[col-1][row-1] == "O" and grid[col-2][row-2] == "S":
            if rounds %2 == 0:
                blue_score += 1
            else:
...
def build_matrix(ROWS_5, COLS_5):
    matrix = []

    for r in range(0, ROWS_5):
        matrix.append([0 for c in range(0, COLS_5)])

    return matrix

grid = build_matrix(6,6)
...
square = pygame.Surface((SQUARE_SIZE_5-5, SQUARE_SIZE_5-5))
...
def game_5x5():
    ...
    board.draw_cubes(board_surface, grid, 5,5)
    while play:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1:
                    i = (mouse[0] - int((WIDTH - BOARD_WIDTH)// 2))//SQUARE_SIZE_5
                    
                    j = (mouse[1]- int((HEIGHT- BOARD_HEIGHT)//2) -50)//SQUARE_SIZE_5
                    if 0 <= i < 5 and 0 <= j < 5 and check_square(i, j):
